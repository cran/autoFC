...
get_CFA_estimates <- function(response_data, fit_model, item_names) {
  ...
  item_loadings <- parameterEstimates(fit_object) %>% filter(.data$op == "=~")
  item_intercepts <- parameterEstimates(fit_object) %>% 
    filter(.data$op == "~1" & (.data$lhs %in% item_names))
  item_residuals <- parameterEstimates(fit_object) %>% 
    filter(.data$op == "~~" & (.data$lhs %in% item_names) & (.data$lhs == .data$rhs))
