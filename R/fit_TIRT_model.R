...
#' 
#' 
#' @param export_estimates Logical. Should trait estimates be written to external files?
#' 
#' @param file_name If \code{export_estimates == TRUE}, specify the file name for the output file.
#'
#' @param remove_mplus_file Whether the input/output files will be removed after model estimation, when Mplus is chosen as the method of estimating the TIRT model.
#'
#' @details 
...
fit_TIRT_model <- function(data_TIRT, method = "lavaan", lavaan_estimator = "WLSMV", 
                           ...
                           export_estimates = TRUE, file_name) {
  FC_results <- list()
  ...
  if (method == "lavaan") {
    ...
    # print("Fitting TIRT")
    fit_TIRT <- fit_TIRT_mplus_new(data_TIRT, remove_mplus_file)
  ...
  if (export_estimates) {
     if (missing(file_name)) {
        write.csv(FC_results$final_estimates, paste0("Estimates_", as.numeric(Sys.time()),".csv"))
     }
     else {
        write.csv(FC_results$final_estimates, file_name)
     }
